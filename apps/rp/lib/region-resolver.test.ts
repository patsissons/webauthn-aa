...
describe("region resolver strategies", () => {
  ...
  it("demo honors the dropdown override, else falls back to RP_REGION", () => {
    expect(
      pickRegion({ kind: "demo", rpRegion: "region-1", regions, demoOverride: "region-2" }).id,
    ).toBe("region-2");
  ...
  it("location uses the region hint, else falls back", () => {
    expect(
      pickRegion({ kind: "location", rpRegion: "region-1", regions, regionHint: "region-2" }).id,
    ).toBe("region-2");
