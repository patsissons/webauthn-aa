// Obviously-fake "ID card" fixtures for exercising the image-evidence /
// age-verification review path. These are deliberately, unmistakably fake:
// a SPECIMEN watermark, a fictional issuer, a silhouette (no real face), and a
// "NOT A VALID IDENTITY DOCUMENT" footer. Never use these to represent a real
// person or a real credential — they exist only as test evidence.
...
/** Encode a fake ID card as a data URL suitable for the `photoDataUrl` ingest. */
...
// Ready-made specimens. DOBs are fixed (not computed from "now") so tests stay
// deterministic; both sit far from the 18-year boundary so the reviewer's
// pass / fail decision is never ambiguous.
//   ADULT_SPECIMEN  — born 1995-03-14 → comfortably over 18 (accept path)
//   MINOR_SPECIMEN  — born 2012-08-09 → comfortably under 18 (reject path)
...
export const ADULT_SPECIMEN_DATA_URL = fakeIdDataUrl(ADULT_SPECIMEN_OPTS);
export const MINOR_SPECIMEN_DATA_URL = fakeIdDataUrl(MINOR_SPECIMEN_OPTS);
